...
[Tailcat](https://github.com/tailscale/tailcat) is Tailscale's open-source CLI for encrypted peer-to-peer
connections that need no account, tailnet, or VPN. Orca can share a server through it, so a client on any
network reaches the server without a private network path.

Install the `tailcat` CLI on both computers (`brew install tailcat`, or a release from the Tailcat
repository). Then either:

- **Desktop app:** under **Advertise this app as a server**, choose **Anywhere via Tailcat** and generate
  the link.
- **`orca serve`:** add `--tailcat`:

```bash
# macOS
orca serve --tailcat
# Linux
orca-ide serve --tailcat
```

The pairing link carries the server's Tailcat address, and the client dials it through its own `tailcat`
CLI. The runtime's WebSocket listener stays on loopback; only the tunnel reaches it. Orca keeps a
persistent Tailcat key under its data directory, so the link survives restarts, and it restarts the tunnel
on launch while any Tailcat link is still granted.

<Callout title="Trust boundary">
  The Tailcat address in the link is a bearer capability: anyone holding the link can reach the
  tunnel, and the E2EE device token then decides whether they get in. Revoking the grant is what
  closes access. Connections bootstrap through Tailcat's public relay and upgrade to a direct path
  when NAT allows; the relay carries only WireGuard ciphertext.
</Callout>
